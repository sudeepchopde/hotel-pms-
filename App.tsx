...
const App: React.FC = () => {
  ...
  const handleLogin = (loggedInUser: UserResponse) => {
    ...
    // Default to frontdesk if allowed/admin
      ...
      try {
        ...
        const [
          ...
          Promise.all([
            ...
            import("./api")
              .then((m) => m.fetchRoomStatuses())
              .catch(() => []) as Promise<RoomStatus[]>,
          ]),
          timeoutPromise,
        ])) as [
          Hotel[],
        ...
        const effectiveHotels =
          hotelsData && hotelsData.length > 0 ? hotelsData : HOTELS;
        ...
        const bookingEvents = (bookingsData || []).map(
        ...
        import("./api")
          .then((m) => m.fetchNotifications(true))
          .then((data) => {
          ...
          .catch((e) =>
            console.error("Failed to fetch notifications on error", e),
          );
      } finally {
        setIsLoading(false);
      }
        ...
        if (data && data.length > 0) {
          ...
          if (latest.id !== lastNotificationIdRef.current) {
            ...
            if (activeTabRef.current === "kitchen") {
              ...
              const isKitchenRelated =
                latest.category === "service_order" &&
                (latest.title?.includes("F&B") ||
                  latest.title?.includes("Dining") ||
                  latest.title?.includes("Kitchen"));
    ...
    const pollBookings = setInterval(async () => {
      try {
        ...
        setSyncEvents((prev) => {
          ...
          const hasChanges =
            JSON.stringify(prev.filter((e) => e.type === "booking")) !==
    ...
    if (notificationToast) {
      try {
        ...
        audio
          ...
          .catch((e) =>
            console.warn(
              "Audio play failed (user interaction required first)",
              e,
            ),
          );
      } catch (e) {
  ...
  const handleValidateGuest = async (
    roomNumber: string,
    ...
    if (booking) {
      if (extractLastName(booking.guestName) === sanitizedInput) {
        ...
        const matchedAccessory = booking.accessoryGuests?.find(
          (guest) =>
            guest.name && extractLastName(guest.name) === sanitizedInput,
        );
    ...
    if (verified) {
      ...
      setRoomSecurity((prev) =>
        prev.map((rs) =>
          rs.room_id === roomNumber ? { ...rs, failCount: 0 } : rs,
        ),
      );
  ...
  const handleSecurityThrottling = (room: string) => {
    setRoomSecurity((prev) => {
      ...
      if (newCount >= 3) {
        setSecurityToast(
          `Security Alert: High failure rate on Room ${room}. Potential fraud detected.`,
        );
        ...
        if (existing) {
          return prev.map((rs) =>
            rs.room_id === room
              ? { ...rs, failCount: newCount, isLocked: true }
              : rs,
          );
        } else {
          return [
      ...
      if (existing) {
        return prev.map((rs) =>
          rs.room_id === room ? { ...rs, failCount: newCount } : rs,
        );
      } else {
        return [
          ...prev,
  ...
  const handleSecurityAlert = (room: string) => {
    ...
    setRoomSecurity((prev) => {
      ...
      if (existing) {
        return prev.map((rs) =>
          rs.room_id === room ? { ...rs, failCount: 3, isLocked: true } : rs,
        );
      }
      return [
        ...prev,
        { room_id: room, isLocked: true, isQRDisabled: false, failCount: 3 },
      ];
    });
    setSecurityToast(
      `Warning: Room ${room} identity verification failed 3 times. Locked.`,
    );
    setTimeout(() => setSecurityToast(null), 5000);
  };
  ...
  const resetRoomSecurity = (room: string) => {
    setRoomSecurity((prev) =>
      prev.map((rs) =>
        rs.room_id === room ? { ...rs, isLocked: false, failCount: 0 } : rs,
      ),
    );
  ...
  const toggleQROrdering = (room: string) => {
    setRoomSecurity((prev) => {
      ...
      if (existing) {
        return prev.map((rs) =>
          rs.room_id === room ? { ...rs, isQRDisabled: !rs.isQRDisabled } : rs,
        );
      }
      return [
        ...prev,
        { room_id: room, isLocked: false, isQRDisabled: true, failCount: 0 },
      ];
    });
  };
  ...
  const handlePlaceOrder = async (
    ...
    const booking = [...syncEvents]
      ...
      .sort(
        (a, b) =>
          ((b as Booking).timestamp || 0) - ((a as Booking).timestamp || 0),
      )[0] as Booking;
    ...
    setSyncEvents((prev) =>
      prev.map((e) =>
        e.id === booking.id && e.type === "booking"
    ...
    try {
      ...
      const bookingsData = await fetchBookings();
      setSyncEvents(
        bookingsData.map((b) => ({ ...b, type: "booking" }) as SyncEvent),
      );
    }
  };
  ...
  const handleUpdateExtraBeds = async (bookingId: string, count: number) => {
    const booking = syncEvents.find(
      (e) => e.type === "booking" && e.id === bookingId,
    ) as Booking | undefined;
    if (!booking) return;
    ...
    const existingIdx = newFolio.findIndex(
      (fi) => fi.category === "Other" && fi.description.includes("Extra Bed"),
    );
      ...
      return (
        ...
        >
          <GuestMenu
            ...
            onPlaceOrder={(_room, items) =>
              handlePlaceOrder(guestRoomNumber, items)
            }
            ...
            <div className="p-8 space-y-6">
              <div className="text-center">
                ...
                <p className="text-slate-500 text-sm mt-3">
                  Room service is only available for guests with an active
                  check-in.
                </p>
              </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="text-sm text-amber-800">
                    ...
                    <p className="mt-1 text-amber-700">
                      Please contact the front desk to complete your check-in
                      process.
                    </p>
                  </div>
                </div>
  ...
  return (
    <div className="h-screen flex flex-col md:flex-row bg-[#fbfcfd] font-inter antialiased overflow-hidden">
        ...
        <div className="relative pt-2 w-full">
          ...
          >
            <div className="flex items-center gap-3 overflow-hidden">
              <div
                className={`p-2 rounded-lg bg-${selectedHotel.color}-500/20 text-${selectedHotel.color}-400 shrink-0`}
              >
      ...
      <main
        className={`flex-1 flex flex-col min-h-0 bg-[#fbfcfd] relative ${activeTab === "frontdesk" ? "overflow-hidden" : "overflow-y-auto"}`}
      >
        {securityToast && (
