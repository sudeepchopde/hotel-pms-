...
const InventoryDashboard: React.FC<DashboardProps> = ({
  hotelId,
  connections,
  rules,
  roomTypes,
  ...
  const calculateFinalRate = (
    ...
    dateStr: string,
    roomTypeId: string,
  ): { price: number; rule: string | undefined } => {
    ...
    const clampedPrice = Math.max(
      rt.floorPrice,
      Math.min(rt.ceilingPrice, finalPrice),
    );
  ...
  useEffect(() => {
    ...
    const rateUpdates = syncEvents.filter(
      (e) => e.type === "rate_update" && e.date,
    );
    rateUpdates.forEach((e) => {
      if (e.type === "rate_update" && e.date) {
        const item = initialInventory.find(
          (i) => i.date === e.date && i.roomTypeId === e.roomTypeId,
        );
        if (item) {
    ...
    const occupiedBookings = syncEvents.filter(
      (e) =>
        e.type === "booking" &&
        (e.status === "Confirmed" ||
          e.status === "CheckedIn" ||
    ...
    occupiedBookings.forEach((b) => {
      ...
      while (d < end) {
        ...
        const item = initialInventory.find(
          (i) => i.date === dateStr && i.roomTypeId === b.roomTypeId,
        );
        if (item) {
    ...
    addLog(
      "INFO",
      `Atomic Engine: Inventory synchronized with ${rules.specialEvents.length} priority overrides.`,
    );
  ...
  const retryChannelSync = async (
    ...
    addLog(
      "WARNING",
      `RETRY [${currentAttempt}/${MAX_AUTO_RETRIES}]: Re-dispatching to ${channel}...`,
    );
    ...
    if (Math.random() < 0.2) {
      if (currentAttempt < MAX_AUTO_RETRIES) {
        ...
        setTimeout(
          () => retryChannelSync(eventId, channel, true),
          AUTO_RETRY_DELAY,
        );
      } else {
  ...
  const simulateFanOut = async (
    ...
    activeChannels.forEach(async (channel) => {
      ...
      if (channel.isStopped) {
        ...
        addLog(
          "WARNING",
          `CIRCUIT BREAKER: Sync skipped for ${channel.name} (Stop-Sell Active).`,
        );
        return;
      }
      ...
      addLog(
        "INFO",
        `PIPELINE: Sending ${finalPriceLabel} to ${channel.name}...`,
      );
  ...
  const simulateRateSync = async (
    ...
    const roomName =
      roomTypes.find((rt) => rt.id === roomTypeId)?.name || roomTypeId;
    const label = `Rate (₹${newPrice.toLocaleString()})${ruleApplied ? ` [via ${ruleApplied}]` : ""}`;

    addLog(
      "INFO",
      `BROADCAST: Dispatching Yield Rate (Base: ₹${newPrice.toLocaleString()}) for ${roomName}.`,
    );
  ...
  const handleBookingTransaction = async (
    ...
    if (otaConn && otaConn.isStopped) {
      addLog(
        "ERROR",
        `GATEWAY REJECT: Ignoring booking from ${source} (Stop-Sell active).`,
      );
      return false;
    }
    ...
    const itemsToBook = currentInv.filter(
      (item) =>
        item.roomTypeId === roomTypeId && datesToBook.includes(item.date),
    );

    const isAvailable =
      itemsToBook.length === datesToBook.length &&
      itemsToBook.every((i) => i.availableCount > 0);

    if (!isAvailable) {
      addLog(
        "ERROR",
        `CONFLICT: Inventory collision detected. Overbooking prevented for ${source}.`,
      );
      return false;
    }
    ...
    if (rt) {
      ...
      const overlappingBookings = eventsRef.current.filter(
        (e) =>
          e.type === "booking" &&
          (e.status === "Confirmed" ||
            e.status === "CheckedIn" ||
      ...
      const occupiedRooms = new Set(
        overlappingBookings.map((b) => b.roomNumber),
      );
    ...
    addLog(
      "SUCCESS",
      `ATOMIC LOCK: Secured Room ${assignedRoomNumber} for #TX-${shortId}.`,
    );
    ...
    setSyncEvents((prev) => [
      ...prev,
      { ...newBooking, type: "booking" } as SyncEvent,
    ]);

    createBooking(newBooking).catch((err) => {
      addLog(
        "ERROR",
        `PERSISTENCE FAIL: Could not save new booking to database.`,
      );
    });
    ...
    setTimeout(() => {
      setInventory((prev) =>
        prev.map((i) =>
          datesToBook.includes(i.date) && i.roomTypeId === roomTypeId
            ? { ...i, isLocked: false }
            : i,
        ),
      );
  ...
  const handleCancellation = async (bookingId: string) => {
    ...
    addLog(
      "WARNING",
      `WEBHOOK: CANCELLATION for Booking #${shortId}. Releasing inventory...`,
    );
  ...
  const simulateRandomCancellation = () => {
    const confirmedBookings = syncEvents.filter(
  ...
  const handleFrontDeskSubmit = async (e: React.FormEvent) => {
    ...
    setIsSyncing(true);
    const success = await handleBookingTransaction(
      "Direct",
      selectedRoomType,
      fdCheckIn,
      fdCheckOut,
  ...
  const runParallelLoadTest = async () => {
    ...
    const dates: string[] = Array.from(
      new Set<string>(inventory.map((i) => i.date)),
    ).sort();
  ...
  const handleUpdatePrice = useCallback(
    (roomTypeId: string, newPrice: number) => {
      simulateRateSync(roomTypeId, newPrice);
    },
    [roomTypes, rules],
  );

  const handleUpdateDailyPrice = useCallback(
    (roomTypeId: string, date: string, newPrice: number) => {
      // Local optimistic update
      setInventory((prev) =>
        prev.map((item) =>
          item.roomTypeId === roomTypeId && item.date === date
            ? { ...item, price: newPrice, appliedRule: "Manual Override" }
            : item,
        ),
      );
      // Dispatch event
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            Markups and Stop-Sells are evaluated globally during sync.
          </p>
        </div>
          ...
          {stoppedChannels.length > 0 && (
            <div className="flex items-center gap-2 px-4 py-2 bg-amber-50 border border-amber-200 text-amber-700 rounded-xl animate-in fade-in zoom-in">
              ...
              <span className="text-xs font-bold">
                {stoppedChannels.length} Channels Blocked
              </span>
            </div>
          )}
          <button
            disabled={
              isSyncing || availableDates.length < 2 || roomTypes.length === 0
            }
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          <div className="bg-slate-900 rounded-3xl border border-slate-800 shadow-xl flex flex-col h-[350px] overflow-hidden">
            ...
            <div
              ref={logContainerRef}
              className="flex-1 overflow-y-auto p-4 space-y-2 font-mono text-[9px]"
            >
              {logs.map((log) => (
                <div key={log.id} className="flex gap-2">
                  <span className="text-slate-600">{log.timestamp}</span>
                  <span
                    className={`font-bold ${log.level === "SUCCESS" ? "text-emerald-400" : log.level === "ERROR" ? "text-red-400" : log.level === "WARNING" ? "text-amber-400" : "text-indigo-400"}`}
                  >
                    {log.level}:
                  </span>
          ...
          <div className="bg-white rounded-3xl border border-slate-200/60 shadow-sm flex flex-col h-[450px] overflow-hidden">
            <div className="p-4 border-b border-slate-100 bg-slate-50 font-bold text-slate-800 text-[10px] uppercase tracking-widest">
              Global Propagation
            </div>
            <div className="overflow-y-auto flex-1 divide-y divide-slate-100/50">
              {syncEvents
                ...
                .map((event) => (
                  <div
                    key={event.id}
                    className="p-5 space-y-3 hover:bg-slate-50/50 transition-colors group/item"
                  >
                    <div className="flex justify-between items-start text-[11px] font-bold">
                      <div className="flex items-center gap-2">
                        {event.type === "booking" ? (
                          <>
                            <span
                              className={`${event.status === "Cancelled" ? "text-slate-400 line-through" : "text-slate-800"}`}
                            >
                              #TX-{event.id.split("-")[1]?.slice(-6) || "??"}{" "}
                              from{" "}
                          ...
                          <div className="flex items-center gap-1.5 text-fuchsia-600 uppercase">
                            ...
                            {event.ruleApplied && (
                              <span className="text-[8px] bg-fuchsia-50 text-fuchsia-500 px-1 rounded ml-1">
                                {event.ruleApplied}
                              </span>
                            )}
                          </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {connections
                        ...
                        .map((ch) => {
                          ...
                          const specificPrice =
                            event.type === "rate_update"
                              ? event.channelPrices?.[ch.name]
                              : null;
                          ...
                          if (status === "success")
                            ...
                            statusClasses =
                              "bg-indigo-500 text-white border-indigo-600 animate-pulse shadow-sm";
                          else if (status === "waiting_retry")
                            statusClasses =
                              "bg-amber-500 text-white border-amber-600 animate-pulse shadow-sm";
                          else if (status === "pending")
                            statusClasses =
                              "bg-indigo-400 text-white border-indigo-500 animate-pulse shadow-sm";
                          else if (status === "stopped") {
                            statusClasses =
                              "bg-amber-500 text-white border-amber-600 shadow-sm ring-1 ring-amber-300";
                            statusLabel = "CLOSED";
                          }
                          ...
                          return (
                            <div
                              key={ch.name}
                              className={`px-2.5 py-1 border rounded-lg text-[9px] font-black uppercase tracking-widest transition-all duration-300 ${statusClasses} flex flex-col items-center`}
                            >
                              <span>
                                {statusLabel === "CLOSED"
                                  ? `${ch.name} (CLOSED)`
