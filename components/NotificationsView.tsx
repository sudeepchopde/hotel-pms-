...
import { Bell, CheckCheck, Trash2, Calendar, CreditCard, Users, Home, Settings } from 'lucide-react';
...
const FILTER_TABS: { id: string; label: string; types: NotificationType[] | null }[] = [
    ...
    { id: 'system', label: 'System & Support', types: ['system', 'housekeeping', 'guest_request'] }
];
            ...
            <div className="p-8 pb-4">
                <div className="flex items-center justify-between mb-6">
                    <div>
                        ...
                        <p className="text-slate-500 mt-1 font-medium">Manage alerts and system updates</p>
                    </div>
                    {unreadCount > 0 && (
                        <button
                ...
                <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-none">
                    {FILTER_TABS.map(tab => (
                        <button
                            ...
                            className={`px-4 py-2 text-sm font-semibold rounded-xl whitespace-nowrap transition-all ${activeTab === tab.id
            ...
            <div className="flex-1 overflow-y-auto px-8 pb-8 space-y-3">
                {loading ? (
                    ...
                    filteredNotifications.map(notification => (
                        <div
                            key={notification.id}
                            className={`group relative p-4 rounded-2xl border transition-all duration-200 ${notification.isRead
                        ...
                        >
                            <div className="flex gap-4">
                                {/* Icon */}
                                <div className={`flex-shrink-0 w-12 h-12 rounded-xl flex items-center justify-center ${TYPE_STYLES[notification.type]}`}>
                                ...
                                <div className="flex-1 min-w-0 py-0.5">
                                    <div className="flex items-start justify-between gap-2">
                                        <h3 className={`text-base font-bold ${notification.isRead ? 'text-slate-600' : 'text-slate-800'}`}>
                                            {notification.title}
                                        </h3>
