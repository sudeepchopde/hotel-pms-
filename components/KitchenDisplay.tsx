...
const KitchenDisplay: React.FC = () => {
  ...
  const loadOrders = async () => {
    try {
      ...
      const isKitchenOrder = (n: any) =>
        ...
        (n.title?.includes("F&B") ||
          n.title?.includes("Dining") ||
          n.title?.includes("Kitchen"));
      ...
      const newKitchenTickets = activeServerOrders.map((o) => {
        ...
        const seconds =
          (new Date().getTime() - new Date(o.createdAt).getTime()) / 1000;
      ...
      if (!isFirstLoad.current) {
        const newIds = newKitchenTickets.filter(
  ...
  useEffect(() => {
    loadOrders();
    const pollInterval = setInterval(loadOrders, 60000); // Sync new orders every 60s
  ...
  const updateTicketStatus = (id: string, newStatus: TicketStatus) => {
    ...
    setOrders((prev) =>
      prev.map((o) => (o.id === id ? { ...o, kitchenStatus: newStatus } : o)),
    );
  };
  ...
  const allDayCounts = useMemo(() => {
    ...
    orders.forEach((t) => {
      const rawItems = t.metadata?.items as
        | { name: string; quantity: number }[]
        | undefined;
      if (rawItems && Array.isArray(rawItems)) {
        // Filter out service charge from All Day Count
        const items = rawItems.filter(
          (i) => !i.name.toLowerCase().includes("service charge"),
        );
  ...
  const getUrgencyClass = (seconds: number, status: TicketStatus) => {
    ...
    if (seconds > 30 * 60)
      return "border-rose-600 bg-rose-950/20 animate-pulse-slow"; // Red > 30m
    ...
    <div className="h-full bg-slate-950 text-slate-100 font-inter flex flex-col overflow-hidden">
      ...
      <header className="h-16 shrink-0 bg-slate-900 border-b border-slate-800 flex items-center justify-between px-6 z-10">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-lg font-black tracking-tight text-white uppercase hidden md:block">
            Kitchen Display
          </h1>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-4 items-start">
                {orders
                  .filter(
                    (o) =>
                      statusFilter === "all" ||
                      o.kitchenStatus === statusFilter,
                  )
                  .map((order) => {
                    // Parse Items
                    const rawItems = (order.metadata?.items || [
                      { name: order.message, quantity: 1 },
                    ]) as { name: string; quantity: number; notes?: string }[];
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="p-3 bg-slate-950/40 flex justify-between items-start border-b border-white/5">
                          ...
                          <div
                            className={`px-2 py-1 rounded bg-black/40 font-mono text-lg font-bold ${order.elapsedSeconds > 1800 ? "text-rose-500 animate-pulse" : "text-slate-300"}`}
                          >
                          ...
                          {items.map((item, idx) => (
                            <div key={idx} className="flex flex-col gap-1">
                              <div className="flex gap-3 text-sm">
                                <span className="font-black text-slate-400 min-w-[20px]">
                                  {item.quantity}
                                </span>
                                <span
                                  className={`font-bold leading-snug ${order.kitchenStatus === "ready" ? "text-emerald-300 line-through opacity-50" : "text-slate-200"}`}
                                >
                                  {item.name}
                                </span>
                              </div>
                        ...
                        <div className="p-2 bg-slate-950/50 flex gap-2">
                          {order.kitchenStatus === "new" && (
                            <button
                              onClick={() =>
                                updateTicketStatus(order.id, "cooking")
                              }
                          ...
                          {order.kitchenStatus === "cooking" && (
                            <button
                              onClick={() =>
                                updateTicketStatus(order.id, "ready")
                              }
              ...
              {completedOrders.length === 0 ? (
                ...
                completedOrders.map((order) => {
                  ...
                  return (
                    <div
                      key={order.id}
                      className="bg-slate-800 border border-slate-700 p-5 rounded-xl flex items-center justify-between shadow-lg px-6"
                    >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-800 p-3 rounded-lg text-center">
                  ...
                  <p className="text-2xl font-black text-white">
                    {orders.filter((o) => o.kitchenStatus === "new").length}
                  </p>
                </div>
                <div className="bg-slate-800 p-3 rounded-lg text-center">
                  <p className="text-[10px] uppercase font-bold text-slate-500">
                  ...
                  <p className="text-2xl font-black text-blue-400">
                    {orders.filter((o) => o.kitchenStatus === "cooking").length}
                  </p>
                </div>
              </div>
            </div>
