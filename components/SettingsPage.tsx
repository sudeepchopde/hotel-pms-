...
const SettingsPage: React.FC<SettingsPageProps> = ({
  ...
  const toggleStopSell = (id: string) => {
    setConnections((prev) =>
      prev.map((c) => (c.id === id ? { ...c, isStopped: !c.isStopped } : c)),
    );
  };
  ...
  const handleDisconnect = (id: string) => {
    setConnections((prev) =>
      prev.map((c) =>
        c.id === id
          ? { ...c, status: "disconnected", key: "", lastValidated: undefined }
          : c,
      ),
    );
  };
    ...
    <div className="p-8 space-y-10 animate-in fade-in duration-500 pb-24">
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-slate-500">
            Securely manage your distribution network and API credentials.
          </p>
        </div>
        <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {MARKETPLACE_OTAS.map((ota) => {
              const isActive = connections.some(
                (c) => c.id === ota.id && c.status === "connected",
              );
              return (
                <div
                  key={ota.id}
                  className={`p-6 rounded-3xl border transition-all ${
                    isActive
                      ...
                      : "bg-slate-700 border-slate-600/50 hover:border-indigo-400/50 group cursor-pointer shadow-sm"
                  }`}
                >
      ...
      <div className="grid gap-8">
        ...
        {connections.map((ota) => (
          <div
            key={ota.id}
            className={`bg-white rounded-[2.5rem] border border-slate-200/60 shadow-sm overflow-hidden transition-all hover:shadow-md ${ota.isStopped ? "ring-2 ring-amber-400/50 bg-amber-50/10" : ""}`}
          >
                ...
                <div className="md:w-48 flex-shrink-0 flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-1.5 mt-1">
                      <div
                        className={`w-2 h-2 rounded-full ${ota.isStopped ? "bg-amber-500 shadow-[0_0_8px_rgba(245,158,11,0.4)]" : ota.status === "connected" ? "bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.4)]" : "bg-slate-300"}`}
                      ></div>
                ...
                <div className="bg-indigo-50/30 border border-indigo-100/50 rounded-3xl p-6 flex flex-col lg:flex-row lg:items-center justify-between gap-6 animate-in slide-in-from-top-2">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h4 className="text-sm font-black text-slate-900 tracking-tight uppercase">
                        Strategic Channel Markup
                      </h4>
                      <p className="text-xs text-slate-500">
                        Adjust price sent to {ota.name} to cover commission
                        costs.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3">
                    <div className="flex bg-white p-1 rounded-xl border border-slate-200 shadow-sm">
                      ...
                      <button
                        onClick={() =>
                          updateMarkup(ota.id, "fixed", ota.markupValue || 0)
                        }
                ...
                <div className="bg-amber-100/30 border border-amber-200 rounded-3xl p-6 flex items-center gap-6 animate-in slide-in-from-top-2">
                  ...
                  <div>
                    <h4 className="text-sm font-black text-amber-900 uppercase">
                      Sales Circuit Breaker Active
                    </h4>
                    <p className="text-xs text-amber-700/80 mt-1">
                      No inventory or rates are being pushed to {ota.name}. All
                      incoming booking requests from this source will be ignored
                      or rejected by the gateway.
                    </p>
                  </div>
                </div>
              )}
      ...
      <div className="bg-amber-50/50 border border-amber-100/60 rounded-[2rem] p-8 flex gap-6">
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm text-amber-700/80 leading-relaxed">
            New OTA connections utilize **KDF isolation**. Tokens are encrypted
            with unique salts, ensuring that channel breaches never propagate.
            Background health checks run every 15 mins.
          </p>
        </div>
      </div>
    </div>
