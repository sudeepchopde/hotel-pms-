...
const GuestsView: React.FC<GuestsViewProps> = ({
  syncEvents,
  setSyncEvents,
  roomTypes,
  onUpdateExtraBeds,
  ...
  const filteredGuests = useMemo(() => {
    return guestList.filter((g) => {
      ...
      // Filter logic
      if (filters.status.length > 0 && !filters.status.includes(g.status))
        return false;
      if (
        filters.roomTypeId.length > 0 &&
        !filters.roomTypeId.includes(g.roomTypeId)
      )
        return false;
      if (filters.source.length > 0 && !filters.source.includes(g.source))
        return false;
      if (filters.isVIP !== null && g.isVIP !== filters.isVIP) return false;
      if (filters.isSettled !== null && g.isSettled !== filters.isSettled)
        return false;
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-500 mt-1 font-medium">
            Full historical archive with room-specific attribution and accessory
            guest details.
          </p>
        </div>
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={() =>
                          setFilters({
                            ...filters,
                            isVIP: filters.isVIP === true ? null : true,
                          })
                        }
                      ...
                      >
                        <Star
                          className={`w-5 h-5 ${filters.isVIP === true ? "fill-white" : ""}`}
                        />
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={() =>
                          setFilters({
                            ...filters,
                            isSettled: filters.isSettled === true ? null : true,
                          })
                        }
                      ...
                      <button
                        onClick={() =>
                          setFilters({
                            ...filters,
                            isSettled:
                              filters.isSettled === false ? null : false,
                          })
                        }
                  ...
                  <div className="flex p-1 bg-slate-50 rounded-2xl">
                    {["All", "Indian", "Foreigner"].map((nat) => (
                      <button
                        key={nat}
                        onClick={() =>
                          setFilters({ ...filters, nationality: nat as any })
                        }
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                      <label className="text-[8px] font-bold text-slate-400 uppercase tracking-wider ml-1">
                        From Arrival
                      </label>
                      <input
                        type="date"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          ...
          },
          {
            ...
            value: guestList.filter((g) => g.status === "CheckedIn").length,
            icon: Bed,
          ...
          },
          {
            ...
            value: guestList.filter((g) => g.status === "Rejected").length,
            icon: ShieldAlert,
          ...
          {
            label: "VIP Guests",
            value: guestList.filter((g) => g.isVIP).length,
            icon: Star,
          ...
          <div
            key={i}
            className="bg-white p-6 rounded-3xl border border-slate-200/60 shadow-sm flex items-center gap-4"
          >
                  ...
                  >
                    ...
                    <td className="p-5">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 bg-slate-100 rounded-2xl flex items-center justify-center text-slate-400 shrink-0 group-hover:bg-indigo-100 group-hover:text-indigo-600 transition-colors relative">
                          ...
                          {guest.isVIP && (
                            <div className="absolute -top-1 -right-1 bg-violet-500 p-1 rounded-full border-2 border-white shadow-sm">
                        ...
                        <div>
                          ...
                          <p className="text-[9px] font-bold text-slate-400 uppercase mt-0.5 flex items-center gap-1.5">
                            <Users className="w-3 h-3" /> {guest.pax || 2} Pax •{" "}
                            {guest.source}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="space-y-1.5">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] font-bold text-slate-700 truncate max-w-[100px]">
                            {getRoomTypeName(guest.roomTypeId)}
                          </span>
                        </div>
                        {guest.extraBeds && (
                    ...
                    <td className="p-5">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2 text-xs font-bold text-slate-700">
                          ...
                          <span>
                            {guest.guestDetails?.idNumber || "No ID Ref"}
                          </span>
                        </div>
                        <div className="flex items-center gap-2 text-[10px] font-bold text-slate-400">
                          <Smartphone className="w-3 h-3" />
                          <span>
                            {guest.guestDetails?.phoneNumber || "N/A"}
                          </span>
                        </div>
                        {guest.accessoryGuests &&
                          guest.accessoryGuests.length > 0 && (
                    ...
                    <td className="p-5">
                      <div className="flex justify-center">
                        <span
                          className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-sm transition-all ${getStatusStyles(guest.status)}`}
                        >
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center gap-8">
          ...
          <div className="space-y-2">
            <h3 className="text-2xl font-black tracking-tight uppercase">
              Strategic Profile Sync
            </h3>
            <p className="text-indigo-100/80 text-sm max-w-2xl leading-relaxed">
              Booking attribution is tracked down to the individual room level.
              Accessory guest identifiers and special requirements are indexed
              for police reporting compliance and operational personalization.
            </p>
          </div>
        </div>
      </div>
