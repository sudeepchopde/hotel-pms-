...
const FrontDeskView: React.FC<FrontDeskViewProps> = ({
  ...
  // Track which email-parsed bookings the user has seen (view-based dismissal)
  const [seenAutoBookingIds, setSeenAutoBookingIds] = useState<Set<string>>(
    new Set(),
  );
  const unseenAutoBookings = useMemo(() => {
    ...
    () =>
      assignedBookings
        .filter((b) => {
          ...
          if (b.reservationId) {
            const hasContinuation = assignedBookings.some(
              (other) =>
                other.id !== b.id &&
    ...
    >
      <div className="flex h-full bg-[#f8fafc] font-inter relative overflow-hidden">
        <div className="flex-1 flex flex-col h-full min-w-0 overflow-hidden relative">
          <header className="px-6 py-4 flex items-center justify-between bg-white border-b border-slate-100 z-[60] shadow-sm relative shrink-0">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                <kbd className="hidden lg:inline text-[9px] font-bold text-slate-300 bg-slate-50 border border-slate-200 rounded px-1.5 py-0.5">
                  ⌘K
                </kbd>
              </button>
              <div className="relative">
                <button
                  onClick={() => {
                    setIsAutoBookingsOpen(!isAutoBookingsOpen);
                    // Mark all current auto bookings as seen when opening
                    if (!isAutoBookingsOpen && autoBookings.length > 0) {
                      setSeenAutoBookingIds((prev) => {
                        const next = new Set(prev);
                        autoBookings.forEach((b) => next.add(b.id));
                        return next;
                      });
                    }
                  }}
                ...
                >
                  ...
                  />
                  {unseenAutoBookings.length > 0 ? (
                    <>
                      <span>{unseenAutoBookings.length} New</span>
                    </>
                  ) : (
          ...
          {isSearchOpen && (
            <div
              className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[200] flex items-start justify-center pt-[15vh]"
              onClick={() => {
                setIsSearchOpen(false);
                setSearchQuery("");
              }}
            >
              <div
                className="w-full max-w-xl bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden animate-in fade-in slide-in-from-top-4"
                ...
                <div className="flex items-center gap-3 px-6 py-4 border-b border-slate-100">
                  ...
                  <input
                    ...
                    onKeyDown={(e) => {
                      ...
                      if (
                        e.key === "Enter" &&
                        searchResults[searchHighlightIdx]
                      ) {
                        setSelectedBooking(searchResults[searchHighlightIdx]);
                        setIsSearchOpen(false);
                        setSearchQuery("");
                      }
                      if (e.key === "Escape") {
                    ...
                    placeholder="Search by name, booking ID, room, phone..."
                    className="flex-1 text-sm font-bold text-slate-900 placeholder:text-slate-400 outline-none bg-transparent"
                    autoComplete="off"
                  />
                  <kbd className="text-[9px] font-bold text-slate-300 bg-slate-50 border border-slate-200 rounded px-2 py-1">
                    ESC
                  </kbd>
                </div>
                      ...
                      searchResults.map((b, idx) => {
                        ...
                        return (
                          <div
                            ...
                            className={`px-6 py-4 cursor-pointer transition-all border-b border-slate-50 last:border-0 ${
                          ...
                          >
                            <div className="flex items-center justify-between mb-1">
                              <div className="flex items-center gap-2">
                                ...
                                {b.source !== "Direct" && (
                                  <span className="text-[9px] font-black uppercase tracking-widest text-indigo-500 bg-indigo-50 px-2 py-0.5 rounded-lg">
                                    {b.source}
                                  </span>
                                )}
                              </div>
                  ...
                  <div className="px-6 py-8 text-center">
                    ...
                    <div className="flex items-center justify-center gap-4 mt-3">
                      <span className="text-[9px] font-bold text-slate-300">
                        <kbd className="bg-slate-50 border border-slate-200 rounded px-1.5 py-0.5 mr-1">
                          ↑↓
                        </kbd>{" "}
                        Navigate
                      </span>
