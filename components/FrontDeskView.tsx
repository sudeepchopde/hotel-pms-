            ...
            <div className="flex items-center gap-3">
              <div className="relative">
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${autoBookings.length > 0 ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-100' : 'bg-slate-100 text-slate-400'}`}
                >
                  <Mail className={`w-3.5 h-3.5 ${autoBookings.length > 0 ? 'animate-bounce' : ''}`} />
                  {autoBookings.length > 0 ? (
                    <>
                      <span>{autoBookings.length} New Parsing</span>
                      <span className="w-4 h-4 rounded-full bg-white text-indigo-600 flex items-center justify-center text-[8px] font-black">{autoBookings.length}</span>
                    </>
                  ) : (
